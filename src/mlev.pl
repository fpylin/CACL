#!/usr/bin/perl
#    -rwxrwxr-x 1 frank users 25937 May 29 2019 23:48 mlev.pl*
# 28 -rwxrwxr-x 1 frank frank 27996 Jan 25 2021 00:46 mlev.pl*
# 28 -rwxrwxr-x 1 frank frank 28345 Feb  9 2021 11:57 mlev.pl*
# 32 -rwxrwxr-x 1 frank frank 29156 Aug  5 2021 02:15 mlev.pl*
# 32 -rwxrwxr-x 1 frank frank 31349 Jan  9 2022 01:02 mlev.pl*
...
my $usage = "
$0: A pipeline for evaluating and implementing machine learning models
 ...
 * Testing:
   ...
   ./mlev.pl  test-thres   model_input   test_data # including threshold curve and statistics
 ...
 * Model calibration:
   ./mlev.pl  calibrate   model_input   test_data  [bins=number|N]  # generating calibration curve and calibrated probability
   
 * Make calibrated prediction:
   ./mlev.pl  predict-prob model_input   test_data
...
MAIN: {
	...
	for ($action) {
		...
		/^calibrate$/  and do { @output = &do_calibrate(@ARGV);    last; };
		...
		/^predict-prob$/    and do { @output = &do_predict_prob(@ARGV[0,1]); last; };
...
######################################################################################
sub gen_calibration_prob(\@\@;$) { # Generate calibration probability by histogram methods, with calibration by isotonic regression
	my @x = @{ $_[0] }; # scores of correctly predicted cases
	my @y = @{ $_[1] }; # scores of wrongly predicted cases
	my $nbins = $_[2] // 10 ; # number of bins; default 10
	my $binw = 1.0 / $nbins; # bin width
	my @bins ;
	for ( my $x = ($binw / 2) ; $x<1; $x += $binw ) { push @bins, $x; }
	
	my @cases ;
	push @cases, map { my %a = ( 'p' => $_, 'e' => 1 ); \%a } @x;
	push @cases, map { my %a = ( 'p' => $_, 'e' => 0 ); \%a } @y;
	...
	for my $bin (@bins) {
		...
		my $E = 0;
		for my $case ( @cases ) {
			if ( ( $$case{'p'} >= $bin - ($binw /2) ) && ( $$case{'p'} < $bin + ($binw /2 )) ) {
				$N ++;
				$E ++ if $$case{'e'} != 0 ;
				}
		}
		next if $N ==0;
		$retval{$bin}{bin} = $bin; # bin (defined midpoint)
		$retval{$bin}{bin_s} = $bin - $binw/2 ; # start bin from starting 
		$retval{$bin}{N} = $N; # number
		$retval{$bin}{E} = $E; # events
		$retval{$bin}{p} = $E/$N ; # probability
		$retval{$bin}{pcalib} = $retval{$bin}{p} ; # calibarated probability 
	}
	@bins = grep { exists $retval{$_} } @bins;
	my $nchanges;
	my $iter = 0;
	do	{
		$iter ++;
		$nchanges = 0;
		for (my $i=1; $i<=$#bins; ++$i) {
			my $Nb = $retval{$bins[$i]}{N};
			my $Eb = $retval{$bins[$i]}{E};
			my $pb = $retval{$bins[$i]}{pcalib};
			my $pb_prev = $retval{$bins[$i-1]}{pcalib};
			next if ( $pb_prev <= $pb ) ;
			
			# isotonic rule violated. merging into a block
			$nchanges ++;
			my $i0 = $i - 1; # i0 = first i with same p_calib
			while ($i0 >= 0 ) {
				$Nb += $retval{$bins[$i0]}{N};
				$Eb += $retval{$bins[$i0]}{E};
				last if $i0 > 0 and $retval{$bins[$i0 - 1]}{pcalib} != $retval{$bins[$i - 1]}{pcalib} ;
				--$i0;
			}
			$pb = $Eb / $Nb;
			for (my $k=$i0; $k<=$i; ++$k) {
				$retval{$bins[$k]}{pcalib} = $pb ;
			}
		}
	} while ($nchanges);
	...
	my @output ;
	my @fields = qw(bin bin_s N E p pcalib); 
	push @output, join("\t", @fields)."\n";
	for my $bin ( @bins ) {
		next if ! exists $retval{$bin}{N} ;
		next if ! defined $retval{$bin} ;
		next if $retval{$bin}{N} == 0;
		push @output, join("\t", ( map { $retval{$bin}{$_} // '?' } @fields ) )."\n" ;
	}
	return @output ;
}

###################################################################################
sub do_calibrate { # Main subroutine: model calibration 
	my @args = @_;
	my $calibration_bins = 10;
	@args = grep { ! /^calib-bins=(\d+|N)$/ or ($calibration_bins=$1) } @args;
	my $fn_input_model = shift @args;
	my $fn_test_data = shift @args;
...
#         print $row;
	...
	for my $l (@class_labels) {
		...
		my @calib_lines = gen_calibration_prob(@x, @y, (($calibration_bins eq 'N') ? (scalar(@x)+scalar(@y)) : $calibration_bins ) ); 
		push @Loutput, "$l\t$_" for @calib_lines;
	}

	my $model = SMLModel->new($fn_input_model);
	$model->set_attr('calibration', map { s/\n$//r } @Loutput);
	$model->save();
...
######################################################################################
sub do_predict_prob { # Main subroutine: do calibrated probability prediction
	my @args = @_;
	my $fn_input_model = $args[0];
	my @predictions = do_predict(@args);
	my $model = SMLModel->new($fn_input_model);
	my @calib_lines = $model->get_attr('calibration');
	my %calib_data_lines;
	my %calib_data;
	for my $l (@calib_lines) {
		my ($cl, $rest) = split /\t/, $l, 2;
		push @{ $calib_data_lines{$cl} }, $rest;
	}
	for my $c (keys %calib_data_lines) {
		$calib_data{$c} = TSV->new;
		$calib_data{$c} -> import_data( @{ $calib_data_lines{$c} } );
	}
	my $TSV_pred = TSV->new;
	$TSV_pred -> import_data(@predictions);
	
	for my $cl ( $TSV_pred ->fields() ) {
		ROW: for my $row ( @{ $TSV_pred ->{'data'} } ) {
			my $p = $$row{$cl};
			next if ! exists $calib_data{$cl} ;
			my @calib_rows = @{ $calib_data{$cl}->{'data'} }; 
			for ( my $i=0; $i<scalar(@calib_rows); ++$i ) {	
				my $xs = $calib_rows[$i]{bin_s} ;
				my $xe = ( $i == scalar(@calib_rows)-1 ) ? 1 : $calib_rows[$i+1]{bin_s};
				if ( ($p >= $xs) && ($p < $xe) ) {
					my $ys = $calib_rows[$i]{pcalib} ;
					my $ye = $calib_rows[min($i+1,scalar(@calib_rows)-1)]{pcalib} ;
					my $p1 = ($xe == $xs) ? $p : ($p - $xs) / ($xe - $xs) * ($ye - $ys) + $ys;
# 					print ">> $i\t$p\t$xs\t$xe\t$ys\t$ye\t$p1\t$calib_rows[$i]{pcalib}\n";
					$$row{$cl} = $p1;
					next ROW;
				}
			}
			$$row{$cl} = "\e[1;31m$$row{$cl}\e[0m";
		}
	}
	
	return $TSV_pred -> to_string() ;
}
