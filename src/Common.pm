...
BEGIN {
	...
	@EXPORT      = qw(
		...
		&uniq &cut &paste
		&min &max &argmax &argmin &min_at &mean &median &range
...
sub sort_keys_numeric(\%) { my %hash = %{ $_[0] }  ; return sort { $hash{$a} <=> $hash{$b} } ( keys %hash ) }

sub hbind { # \@keys, \@values
	die "hbind: argument 0 is not an array reference." if ref($_[0]) ne 'ARRAY' ;
	die "hbind: argument 1 is not an array reference." if ref($_[1]) ne 'ARRAY' ;
	my @k = @{ $_[0] };
	my @v = @{ $_[1] };
	warn "hbind: the dimensions of keys and values are different."  if scalar(@k) ne scalar(@v);
	my %a;
	$a { $k[$_] } = $v[$_] for ( 0 .. $#k );
	return %a;
}
...
sub cbind_str{
	...
	for my $r (0 .. $#mat) {
		push @retval, join("\t", ( map { $mat[$r][$_]// '' } (0 .. $nc) ) )."\n";
	}
	return @retval;
}
...
sub touch {
	...
	my $now = time;
	local (*TMP);
	utime ($now, $now, $file) || open (TMP, ">>$file") || warn ("Couldn't touch file: $!\n");
}
